    ...
    const fetchData = async () => {
      try {
        ...
        formik.setValues({
          ...
          jobLocation: invoice.jobLocation || '',
          jobDate: new Date(invoice.jobDate).toISOString().split('T')[0],
          jobStart: invoice.jobStart ? new Date(invoice.jobStart).toISOString().slice(0, 16) : '',
          ...
          workOrderedBy: invoice.workOrderedBy || '',
          jobAddress: {
            location: invoice.jobAddress?.location || '',
            city: invoice.jobAddress?.city || '',
            country: invoice.jobAddress?.country || '',
            postalCode: invoice.jobAddress?.postalCode || '',
          },
        });
